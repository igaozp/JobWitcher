...
class BosszhipinPipeline(object):
    ...
    def insert_data(self, item):
        ...
        """
        sql = 'insert into bosszhipin (job_name, salary, company_name, city, description, address, ' \
              'experience, education, public_date, url, crawled_date) values ' \
    ...
    def process_item(self, item, spider):
        ...
        :return: item 对象
        """
        if spider.name == 'bosszhipin_spider':
            # 通过 Redis 进行 URL 去重
            url = item['url']
            if self.redis_db.hexists('bosszhipin_url', url):
                ...
                self.redis_db.hset('bosszhipin_url', url, 0)
